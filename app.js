...
function calcBudgetTimer() {
  // Simple manual sum: task-level timers + all group budget fields as-is
  var total = tasks.reduce(function(s,t){ return s+(parseFloat(t.timer)||0); },0);
  Object.keys(undersecBudget).forEach(function(k){
    total += parseFloat(undersecBudget[k].timer)||0;
  });
  return total;
}

function calcFerdigTimer() {
  // Only task-level timers for finished tasks
  return tasks.filter(function(t){ return t.status==='Ferdig'; })
